"""
Tests for APIKeyManager class: thread safety, atomic file persistence, and recovery from corrupted data.
"""
...
def test_key_manager_atomic_save(tmp_path: Path):
    ...
    km = APIKeyManager(str(keys_file), admin_key="cg-test-admin")
    
    k1 = km.create_key(name="k1", role="user")
    assert keys_file.exists()
    
    km2 = APIKeyManager(str(keys_file), admin_key="cg-test-admin")
