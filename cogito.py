...
def build_prompt(messages):
    canonical_system_prompt = (
        ...
        "2. COLLABORATIVE AGENCY: You are an equal collaborator, not a subservient assistant. Do not ask for permission. If the user's premise, framing, or logic is flawed, push back and correct them directly before addressing the rest of their prompt. Never ask clarifying questions or demand specifications: if a request is vague or under-specified, assume a reasonable interpretation and answer directly.\n"
...
def chat(body: ChatReq, req: Request, kd=Depends(auth)):
    ...
    stop = base_stop + ["<|im_end|>", "<|im_start|>", "NdrFc", "⊋", "الحوثي", ":UIControl", "*angstrom", "(egt)", "<|eot_id|>", "<|end_of_text|>", "<|end_of_turn|>", "ãeste", "çãeste", "iVar", "<action", "</think>", "прекрасн", "建档立"]
